...
#ifdef DEBUG
static void check(int status, unsigned long fail_value, const char* name, int line, int* numerrsp)
{
...
#define CHECK(status, fail_value, name) {check(status,fail_value,name,__LINE__, &num_errs);}
#else
